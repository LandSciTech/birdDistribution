...
sd_linear <- 1
cov_df <- data.frame(covariate = paste0("PC",1:8), model = "linear",
                     mean = 0, prec = 1/sd_linear^2)
cov_df <- cov_df %>% mutate(beta = 1) %>%
  bind_rows(cov_df %>% mutate(beta = 2, prec =  1/(sd_linear/2)^2))


test_that("overall flow works", {
  mod <- fit_inla(sp_code = test_dat$species_to_model$Species_Code_BSC[1],
  ...
  pred <- predict_inla(dat = test_dat$ONGrid,
                       analysis_data = test_dat,
                       mod = mod,
  ...
  dir_use <- tempdir()
  map_inla_preds(sp_code = test_dat$species_to_model$Species_Code_BSC[1],
                 analysis_data = test_dat,
                 pred,
                 proj_use = AEA_proj,
                 study_poly = test_area,
