...
gee_points_extract <- function(
  ...
  meth.gd <- meth %>% dplyr::filter(Source=="Google Drive"|Source == "SCANFI", Use==1)
  ...
  for(i in loop){
    ...
    # Determine if temporally matching
      ...
      if (meth.gd.i$SpatialResolution[1]<resolution.threshold &
        ...
        rast.sample <- rast(files.i$Link[1])
        loc.sample <- loc.n %>% 
          st_transform(crs(rast.sample)) %>% 
        ...
        loc.buff.loop <- loc.n.loop %>% 
          st_transform(projection.trans) %>% 
          st_buffer(local.buffer) %>%  #buffer width!
          st_transform(crs(rast.sample))
        
        loc.buff2.loop <- loc.n.loop %>% 
          st_transform(projection.trans) %>% 
          st_buffer(neighbour.buffer) %>% #buffer width!
          st_transform(crs(rast.sample)) 
      }
      else {
        loc.n.loop<-loc.n
      ...
      dt = data.table::data.table(year=unique(files.i$year), 
  ...
  write.csv(loc.gd, file = file.path(out_save, "data_covariates_GD.csv"), 
            row.names=FALSE)
...
#'
#' @examples
gd_dl_file <- function(gd_folder, gd_file, gd_dl_dir){
  file_path <- file.path(gd_dl_dir, gd_file)
  if(dir.exists(file_path)){
    return(list.files(file_path, ".tif") %>% {file.path(file_path, .)} %>% list())
  }
  if(file.exists(file_path)){
    return(file_path)
  }
  file_out <- googledrive::drive_get("https://drive.google.com/drive/folders/11KojcXJ4sLSBONhpWGeRVlgPXfy8VDat") %>%
    googledrive::drive_ls() %>% filter(name == gd_folder) %>% 
    googledrive::drive_ls() %>% filter(name == gd_file)
  if(file_out %>% googledrive::is_folder()) {
    new_dir <- file.path(gd_dl_dir, unique(file_out$name))
    if(!dir.exists(new_dir)) dir.create(new_dir)
    if(file_out %>% slice(1) %>% googledrive::is_folder()){
      # For SCANFI will just download everything in the SCANFI folder and then
      # sort out from there
      files_out <- file_out %>% googledrive::drive_ls() %>% 
        filter(!str_detect(name, "DIFFUSION"), !str_detect(name, "nfiLandCover"))
      beepr::beep_on_error(files_out_list <- map(1:nrow(files_out), \(x){
        files_out %>% slice(x) %>% googledrive::drive_ls(pattern = ".tif$") %>% rowwise() %>% 
          ...
          mutate(
            LocalLink = ifelse(
              ...
              googledrive::drive_download(
                id, path = file.path(file_path, name), overwrite = TRUE
              ) %>%
                pull(local_path) 
    ...
    files_out <- file_out %>% googledrive::drive_ls() %>% rowwise() %>%
      mutate(
    ...
    file_out %>% googledrive::drive_download(path = file_path, overwrite = TRUE) %>%
      pull(local_path) %>% 
      return()
  }
}
...
read.files<-function(meth.gd.i, use_GD, gd_dl_dir){
  ...
  if(any(str_detect(meth.gd.i$Label,"LF"))){
    files <- data.frame(Link=list.files(meth.gd.i$Link, full.names = TRUE,
                                        recursive=TRUE, pattern="*.tif"),
                        file=list.files(meth.gd.i$Link, recursive=TRUE,
                                        pattern="*.tif")) %>% 
      separate(file, into=c("year", "region", "name", "tif"), 
               remove=FALSE) %>% 
      mutate(name = case_when(str_detect(name, c("CBD|cbd")) ~ "biomass",
  ...
  #For SCANFI
    ...
    if(use_GD & any(str_detect(meth.gd.i$Link, "^G:"))){
      files <- meth.gd.i %>% 
        ...
        mutate(
          LocalLink = gd_dl_file(gd_folder, gd_file, gd_dl_dir),
          Link = list(LocalLink), 
          file = list(map_chr(Link, \(x){str_extract(x,  pattern = "^(.+/)*(.+\\.tif)$", group = 2)}))
      ...
      files <- data.frame(Link=list.files(meth.gd.i$Link, full.names = TRUE,
                                          ...
                                          pattern= "*tif$"),
                          file=list.files(meth.gd.i$Link, full.names = FALSE,
                                          recursive=TRUE, ignore.case = TRUE, 
                                          pattern="*tif$")) 
    }
    files <- files %>% 
      dplyr::filter(!str_detect(file,"DIFFUSION"),
                    !str_detect(file,"\\((.*?)\\)"))%>% 
      #DIFFUSION is for a repeated version on year 2020
      #then any file that has a copy (1), (2), etc. 
      mutate(year = as.numeric(str_extract(file, "\\d+(?=\\_v)" )),
             Name = case_when(str_detect(file,"VegTypeClass") ~ "scanfilcc",
    ...
    if(use_GD & any(str_detect(meth.gd.i$Link, "G:/Shared drives/BAM_NationalModels"))){
      ...
      files <- files %>% 
        select(Link, file) %>% unnest(cols = c(Link, file))%>% 
  ...
  if(any(str_detect(meth.gd.i$Extraction,"radius"))){ #if it is a radius: Extract point value,
    ...
    if(meth.gd.i$TemporalResolution[1]=="static"){
      loc.cov.i <- loc.n.j %>% 
        ...
        dplyr::select(-cov) %>% 
        # st_transform(crs=projection.trans) %>% # this doesn't do anything
      ...
      loc.cov <- loc.n.j %>% 
        st_transform(crs(rast.i)) %>% 
        exact_extract(x=rast.i, ifelse(meth.gd.i$RadiusFunction == "cv", "coefficient_of_variation", meth.gd.i$RadiusFunction), force_df=TRUE) %>%
        data.table::setnames(meth.gd.i$Label) %>% 
        cbind(loc.n.j %>% 
                st_drop_geometry() %>% 
                dplyr::select(id)) %>% 
